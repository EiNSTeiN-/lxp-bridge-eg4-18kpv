...
async fn handles_read_input_all() {
    ...
    let tf = async {
        ...
        assert_eq!(
            to_mqtt.recv().await?,
            mqtt::ChannelData::Message(mqtt::Message {
                topic: format!("{}/input/fault_code/parsed", inverter.datalog()),
                retain: false,
